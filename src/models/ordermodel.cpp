...
OrderModel::OrderModel( QVector<utilities::OrderResultData> &orders,
                        QObject *parent )
                         ...
                         int role )
{
    ...
    utilities::OrderResultData& order{ orders_[index.row()] };
    switch( index.column() )
    {
    case 0:
...
{
    utilities::OrderResultData const & row_data{ orders_[index.row()] };
...
OrderDetailModel::OrderDetailModel(
        QVector<utilities::OrderingResultItem> const &order_items,
...
QVariant OrderDetailModel::data( QModelIndex const &index, int role ) const
{
    if( role == Qt::DisplayRole ){
        ...
        utilities::OrderingResultItem const & row_data{ order_items_[row_number] };
        switch( index.column() ){
        case 0:
