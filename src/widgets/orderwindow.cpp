...
OrderWindow::OrderWindow( QWidget *parent ): QMainWindow( parent ),
    ui( new Ui::OrderWindow ), orders_page_query{ new utilities::PageResultQuery }
{
    ui->setupUi( this );
...
void OrderWindow::OnConfirmOrderPaymentTriggered()
{
    ...
    utilities::OrderResultData const & order{ orders_[ index.row() ]};
                                    ...
                                    QVector<utilities::OrderResultData> &orders )
{
    ...
    for( auto const &value: order_object_list ){
        ...
        utilities::OrderResultData order {};
