...
ProductUploadDialog::ProductUploadDialog( QVector<utilities::ProductData> &products,
                                          QWidget *parent ) :
...
void ProductUploadDialog::UploadProducts( QJsonArray const & product_list )
{
    ...
    QProgressDialog *progress_dialog{ new QProgressDialog( "Uploading", "Cancel", 1, 100,
                                                           this )};
    progress_dialog->setAttribute( Qt::WA_DeleteOnClose );
    this->connect( progress_dialog, &QProgressDialog::canceled, reply, &QNetworkReply::abort );
    this->connect( reply, &QNetworkReply::uploadProgress, [=]( qint64 received, qint64 total ){
        progress_dialog->setMaximum( total + ( total * 0.25 ) );
        progress_dialog->setValue( received );
    });
    this->connect( reply, &QNetworkReply::finished, progress_dialog, &QProgressDialog::close );
